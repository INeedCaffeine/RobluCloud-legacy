...
module.exports = {
  ...
  pushCheckouts: asyncHandler(function(req, res) {
    ...
    for (item in classmem) {
      ...
      /*
       * Why is 30 seconds added to the time? This is a bit complicated, but let me explain.
       ...
       * in this case, 30 seconds past the current time, to prevent this issue.
       */ 
      try { await(Checkouts.update(query, { content: classmem[item], status: status2, time: (Date.now() / 1000 | 0) + 30})); }
